...
@Suppress("DiscouragedApi", "Deprecation", "Unused")
open class IconPackManager(val pm: PackageManager) {
    ...
    open inner class IconPack(
        ...
        open fun getAllIconResources(): HashMap<String, Drawable> {
            val drawables = hashMapOf<String, Drawable>()
            val distinctIcon = icons.entries.distinctBy { it.value }.associate { it.key to it.value }
        ...
        private fun getDrawable(value: String): Drawable? {
            val resourceId = res.getIdentifier(value, "drawable", packageName)
            if (resourceId > 0) {
                return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                    res.getDrawable(resourceId)
                } else {
                    res.getDrawable(resourceId, null)
                }
            }
            return null
        }

        open fun loadIcon(info: ApplicationInfo): Drawable? {
        ...
        open fun loadIcon(launchIntent: Intent): Drawable? {
        ...
        open fun loadIcon(comp: ComponentName): Drawable? {
